...
function Show-Help {
    ...
    Write-Host "  stop         -> arrête les processus backend/frontend de ce dépôt"
...
switch ($Command.ToLower()) {
    ...
    "dev" {
        & powershell -NoProfile -ExecutionPolicy Bypass -File (Join-Path $RepoRoot "scripts\dev-processes.ps1") -Action Check
        if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
        Write-Host "Lancement backend (:8000) + frontend (:3000) dans 2 fenêtres PowerShell..." -ForegroundColor Cyan
        # Sans --reload : les optimisations longues restent rattachées à un seul
        # processus et la fermeture de la fenêtre ne laisse pas de worker orphelin.
        $backCmd  = "Set-Location $([char]34)$Backend$([char]34); uv run uvicorn app.main:app --host 127.0.0.1 --port 8000"
        $frontCmd = "Set-Location $([char]34)$Frontend$([char]34); npm run dev -- --hostname $([char]34)$FrontendHost$([char]34)"
        $backendWindow = Start-Process powershell -ArgumentList "-NoExit", "-Command", $backCmd -PassThru
        Write-Host "Attente du backend avant le lancement du frontend..." -ForegroundColor DarkGray
        Push-Location $Backend
        try {
            & uv run python scripts/wait_for_health.py --url http://127.0.0.1:8000/api/v1/health --timeout 120
            if ($LASTEXITCODE -ne 0) { throw "Le backend ne répond pas dans le délai imparti." }
        } catch {
    ...
    "stop" {
        & powershell -NoProfile -ExecutionPolicy Bypass -File (Join-Path $RepoRoot "scripts\dev-processes.ps1") -Action Stop
        if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    }
    ...
    default {
        Write-Host ("Commande inconnue : {0}" -f $Command) -ForegroundColor Red
        Show-Help
        exit 1
    }
}
