...
3. Sauvegardes préalables : cache_status.json.bak-<ts> + table SQL
   buffett_run_result_bak_<ts>.
...
def migrate(apply: bool) -> None:
    ...
    if apply:
        bak = CACHE_FILE.with_suffix(f".json.bak-{ts}")
    ...
    if apply:
        cur.execute(f"CREATE TABLE buffett_run_result_bak_{ts.replace('-', '_')} "
                    f"AS SELECT * FROM buffett_run_result WHERE run_id IN (?, ?)",
