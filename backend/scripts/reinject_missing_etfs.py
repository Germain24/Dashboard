...
def main(write: bool = True) -> None:
    ...
    bak_xlsx = bf.with_name(f"{bf.stem}.bak-{ts}{bf.suffix}")
    shutil.copy2(bf, bak_xlsx)
    wb.save(bf)
    ...
    if missing_csv:
        bak_csv = tk_path.with_name(f"{tk_path.stem}.bak-{ts}{tk_path.suffix}")
        shutil.copy2(tk_path, bak_csv)
