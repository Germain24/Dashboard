...
from app.services.finance.buffett.broker_availability import (  # noqa: E402
    _secteur1_col,
    find_broker_file,
)
...
def main(write: bool = True) -> None:
    ...
    if write:
        ...
        bak = backup_file(
            p,
            category="maintenance/classify_etf_sectors",
            filename=f"{Path(p).stem}.bak-{timestamp}.xlsx",
        )
