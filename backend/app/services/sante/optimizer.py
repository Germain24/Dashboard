...
def optimize_nutrition(
    ...
    for name in food_names:
        ...
        max_qty_csv = float(row.get("MaxQty", 0) or 0)
        if max_qty_csv > 0:
            # MaxQty est exprimé en g si > 1, sinon déjà en unités
            csv_max = max_qty_csv / 100.0 if max_qty_csv >= 1.0 else max_qty_csv
    ...
    for i, name in enumerate(food_names):
        if any(s in name for s in STAPLE_KEYWORDS):
            x0[i] = 1.0  # 100g de chaque staple
        else:
            x0[i] = 0.05  # 5g de chaque autre
