...
def get_full_calendar_with_carryover(
    session: Session,
    from_dt: dt.datetime,
    to_dt: dt.datetime,
) -> list[dict[str, Any]]:
    """Vue agenda incluant les événements commencés la veille et encore actifs."""
    current = get_full_calendar(session, from_dt, to_dt)
    previous = get_full_calendar(session, from_dt - dt.timedelta(days=1), from_dt)
    carryover = [
        event for event in previous
        if event.get("fin") and event["fin"] > from_dt and event["debut"] < from_dt
    ]
    return sorted([*carryover, *current], key=lambda event: event["debut"])
