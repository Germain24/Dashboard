...
def purge_old(
    ...
    *,
    ...
    """Supprime les JobRun anciens et les notifications anciennes.

    Deux fenêtres distinctes pour les notifications : les LUES au-delà de
    ``notif_days`` (courte), les NON LUES au-delà de ``unread_days`` (longue).
    Sans cette seconde borne, aucune notification non lue n'était jamais purgée
    et la table croissait sans fin. Retourne le nombre d'éléments supprimés.
    """
    now = now or utcnow()
    ...
    un_days = (
        settings.notification_unread_retention_days if unread_days is None else unread_days
    )

    jr_cut = cutoff(now, jr_days)
    nt_cut = cutoff(now, nt_days)
    un_cut = cutoff(now, un_days)
    ...
    notifs = session.exec(
        select(Notification).where(
            ...
            | (
                (Notification.lu == False) & (Notification.created_at < un_cut)  # noqa: E712
            )
        )
    ).all()
...
def run(session: Session) -> str:
    ...
    return f"Purge : {res['job_runs']} JobRun, {res['notifications']} notifications supprimés."
