...
def prune_backups(backup_dir: Path, keep: int) -> int:
    """Garde les `keep` backups les plus récents (*.db), ou tous si `keep` vaut 0."""
    if keep <= 0:
        return 0
...
def get_backup_directory(configured_dir: str | Path | None, db_path: Path) -> Path:
    """Retourne le dossier NAS configuré, sans repli local."""
...
def run(session):
    ...
    backup_dir = get_backup_directory(getattr(settings, "backup_dir", None), db_path)
    # Ne pas retomber silencieusement dans data/backups si le NAS est absent :
    # l'échec du job signale clairement que la sauvegarde distante n'a pas eu lieu.
    ...
    try:
        keep = max(0, int(preferences.get(
            "backup_retention_count",
            getattr(settings, "backup_retention_count", 0),
        )))
    except (TypeError, ValueError):
        keep = max(0, int(getattr(settings, "backup_retention_count", 0)))
