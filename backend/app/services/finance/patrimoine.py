...
_INVEST_KEYWORDS = ("boursedirect", "trading", "realt", "kraken", "metamask")
_BANK_KEYWORDS = ("banque", "desjardins", "wise", "westpac")


def _current_asset_value_eur(session: Session, *, bank_only: bool = False) -> float:
    """Somme les comptes actifs avec leur dernier solde importé disponible.

    ``bank_only`` conserve uniquement les comptes bancaires et exclut donc les
    comptes-titres/placements. La catégorie reste la source principale ; les
    mots-clés couvrent les anciens comptes créés avant l'ajout des catégories.
    """
    auto_balances = _load_auto_balances()
    total = 0.0
    for item in list_items(session):
        if item.type != "actif":
            continue
        if bank_only:
            identity = _norm_key(f"{item.categorie} {item.label}")
            if not any(keyword in identity for keyword in _BANK_KEYWORDS):
                continue
        auto = _match_auto_balance(item.label, auto_balances)
        if auto is not None:
            valeur = float(auto["solde"])
            devise = auto.get("devise") or item.devise
        else:
            valeur, devise = item.valeur, item.devise
        total += to_eur(valeur, devise)
    return round(total, 2)


def all_account_value_eur(session: Session) -> float:
    """Valeur de tous les comptes actifs, bancaires et d'investissement."""
    return _current_asset_value_eur(session)


def financial_freedom_value_eur(session: Session) -> float:
    """Patrimoine net disponible : actifs moins emprunts et taxes estimées."""
    net = float(net_worth_summary(session)["net"])
    try:
        from app.services.finance.portfolio_state import get_portfolio_state

        taxes = float(get_portfolio_state(session).get("taxes", {}).get("total", 0) or 0)
    except Exception:
        taxes = 0.0
    return round(net - taxes, 2)


def bank_account_value_eur(session: Session) -> float:
    """Valeur des comptes bancaires uniquement, hors investissements."""
    return _current_asset_value_eur(session, bank_only=True)
