...
# Cours fournis par une source de compte externe (Kraken/Blockscout). À la
# différence du cache Yahoo, 0 est une valeur valide (jetons RealT dépréciés).
_external: dict[str, float] = {}
...
_EXTERNAL_PRICE_CACHE_FILE = (
    Path(__file__).resolve().parents[4] / "data" / "cache" / "external_prices.json"
)
...
def load_disk_cache() -> None:
    ...
    try:
        ...
        pass
    try:
        raw_external = json.loads(_EXTERNAL_PRICE_CACHE_FILE.read_text(encoding="utf-8"))
        with _lock:
            _external.update(
                {str(ticker).upper(): max(0.0, float(price)) for ticker, price in raw_external.items()}
            )
    except Exception:
        pass
...
def _refresh_prices(tickers: list[str], today: dt.date) -> None:
    try:
        ...
        if any(price > 0 for price in refreshed.values()):
            # L'état portefeuille peut avoir été calculé avec un cours périmé.
            ...
            publish(
                ...
                data={"kind": "prices", "tickers": sorted(refreshed)},
                invalidate=[["finance", "state"], ["finance", "portfolio"]],
            )
    finally:
        with _lock:
...
def set_external_prices(prices: dict[str, float]) -> None:
    """Publie des cours EUR autoritatifs issus des comptes synchronisés."""
    with _lock:
        for ticker, price in prices.items():
            if ticker:
                _external[ticker.upper()] = max(0.0, float(price or 0))
        persisted = dict(_external)
    try:
        _EXTERNAL_PRICE_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        _EXTERNAL_PRICE_CACHE_FILE.write_text(
            json.dumps(persisted, indent=1, sort_keys=True), encoding="utf-8"
        )
    except Exception:
        pass
