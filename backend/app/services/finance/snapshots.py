...
def monotonic_invested_values(rows: list[SnapshotPortefeuille]) -> list[float]:
    """Retourne le capital investi cumulé sans faux replis de snapshot.

    ``SnapshotPortefeuille.investit`` est historiquement calculé depuis les
    positions ouvertes (coût de revient). Un rafraîchissement partiel pouvait
    donc faire disparaître temporairement des positions et faire chuter cette
    série, sans retrait ni vente correspondante. Pour l'affichage du capital
    engagé, on conserve le maximum observé jusqu'à chaque date.
    """
    peak = 0.0
    values: list[float] = []
    for row in rows:
        peak = max(peak, float(row.investit or 0.0))
        values.append(round(peak, 2))
    return values
...
def take_snapshot_now(session: Session) -> SnapshotPortefeuille | None:
    ...
    try:
        ...
        # Exception : les RealTokens (immobilier tokenisé) sont en liquidation,
        # leur cours est volontairement fixé à 0 (introuvable) ; ils ne doivent
        # donc pas bloquer le snapshot. Leur coût de revient reste compté dans
        # `total_investit`, leur valeur (0) dans `total_valeur`.
        if any(
            float(position.get("quantite", 0) or 0) > 0
            and float(position.get("prix_actuel", 0) or 0) <= 0
            and not str(position.get("ticker", "") or "").upper().startswith("REALTOKEN-")
        ...
        if previous:
            total_investit = max(total_investit, previous.investit)
