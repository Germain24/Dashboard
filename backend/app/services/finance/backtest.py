...
def simulate_walk_forward(
    ...
    if not dates or not prices or not allocations:
        return {
            ...
            "volatility_pct": 0.0, "semi_deviation_pct": 0.0, "omega": 0.0,
        }
    ...
    if daily.size:
        ...
        volatility = float(np.std(daily, ddof=1) * np.sqrt(252.0)) if daily.size > 1 else 0.0
        downside = np.minimum(daily, 0.0)
        semi_deviation = float(np.sqrt(np.mean(downside * downside)) * np.sqrt(252.0))
        gains = float(np.maximum(daily, 0.0).sum())
        losses = float(-np.minimum(daily, 0.0).sum())
    ...
    return {
        ...
        "semi_deviation_pct": round(semi_deviation * 100.0, 2),
