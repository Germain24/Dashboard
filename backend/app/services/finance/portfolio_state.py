...
def _attach_invested_totals(state: dict) -> dict:
    """Ajoute ``investi_total`` (base de coût) et ``pl_total`` (réalisé + latent).

    ``investi_net`` mesure les apports nets (dépôts − retraits), pas l'argent
    réellement placé. Ces deux champs répondent à « combien ai-je investi au
    total, combien ai-je réellement gagné ou perdu » : la base de coût agrégée
    de toutes les positions ouvertes (crypto/RealT inclus) et le gain/perte
    total depuis l'origine.
    """
    investi_total = round(
        sum(
            float(position.get("acb", 0) or 0)
            * float(position.get("quantite", 0) or 0)
            for position in state.get("positions", [])
        ),
        2,
    )
    state["investi_total"] = investi_total
    state["pl_total"] = round(
        float(state.get("pl_realise", 0) or 0)
        + float(state.get("pl_latent_total", 0) or 0),
        2,
    )
    return state
...
def get_portfolio_state(session) -> dict:
    ...
    if not txs:
        ...
        state = _attach_invested_totals(state)
...
def _merge_manual_positions(session, state: dict) -> dict:
    ...
    authoritative_brokers = {"Kraken", "MetaMask · Ethereum", "MetaMask · Gnosis", "MetaMask · Bitcoin"}
    manual_keys = {
        (position.ticker.upper(), position.broker or "default")
        for position in session.exec(select(Position)).all()
        if position.broker in authoritative_brokers or position.broker.startswith("MetaMask · RMM")
    }
    # Pour les comptes synchronisés, le solde API/on-chain est la quantité
    # courante de référence (staking et transferts ne figurent pas toujours
    # dans l'historique des trades Kraken).
    if manual_keys:
        state["positions"] = [
            position for position in state["positions"]
            if (str(position["ticker"]).upper(), position.get("broker") or "default") not in manual_keys
        ]
    existing = {
    ...
    for position in session.exec(select(Position)).all():
        ...
        if key in seen and key not in manual_keys:
            continue
        seen.add(key)
