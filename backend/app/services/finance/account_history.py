...
def _t212_api_points() -> list[tuple[dt.date, float]]:
    """Points (date, valeur du compte) du journal de snapshots de l'API.

    Vide si la clé n'est pas configurée ou qu'aucun snapshot n'a encore été
    enregistré : l'historique retombe alors sur les relevés / le ledger.
    """
    try:
        from app.services.finance import trading212_api as api
        return [(dt.date.fromisoformat(r["date"]), round(float(r["total"]), 2))
                for r in api.load_history() if r.get("date")]
    except Exception:
        return []
...
def account_history_points(*, force: bool = False) -> dict[str, list[tuple[dt.date, float]]]:
    ...
    # Journal de snapshots Trading 212 (API) : un point par jour. Sa taille
    # entre dans la signature pour que le cache s'invalide à chaque nouveau
    # point, sans quoi la courbe resterait figée sur le premier jour.
    t212_api_points = _t212_api_points()
    if t212_api_points:
        sig = sig + [f"t212api:{len(t212_api_points)}:{t212_api_points[-1][0].isoformat()}"]
    ...
    # Trading 212 : le journal de l'API prime (un point par jour, valeur réelle
    # du compte), complété par les relevés PDF pour les dates antérieures à sa
    # mise en service — l'API n'expose aucun historique de valorisation.
    pts: list[tuple[dt.date, float]] = list(t212_api_points)
    api_dates = {d for d, _ in pts}
    for pdf in t212_pdf:
        try:
            p = parse_trading212_statement(extract_pdf_text(pdf.read_bytes()))
            if p["account_value"] and p["date"]:
                d = dt.date.fromisoformat(p["date"])
                if d not in api_dates:
                    pts.append((d, round(float(p["account_value"]), 2)))
        except Exception:
            pass
    # Dernier recours : valeur du jour dérivée du ledger en base. Il ne dépend
    # PAS de la présence des CSV — ceux-ci ne sont qu'une source d'import, et
    # les transactions qu'ils ont alimentées sont persistées en base. Les
    # exiger ferait disparaître Trading 212 de la courbe une fois les exports
    # supprimés, alors même que le ledger est toujours là.
    if not pts:
