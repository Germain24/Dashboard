...
from app.services.finance.snapshots import monotonic_invested_values
...
def get_perf_metrics(session: Session) -> dict:
    ...
    if not snaps:
        return {}
    invested_values = monotonic_invested_values(snaps)
    latest = snaps[-1]
    valeur = latest.valeur
    investit = invested_values[-1]
    ...
    twr = time_weighted_return([
        (s.date, s.valeur, invested)
        for s, invested in zip(snaps, invested_values)
    ])
