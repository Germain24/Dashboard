...
def warm_fx_cache(quote: str = "EUR", budget_s: float = 90.0) -> None:
    ...
    t0 = _time.monotonic()
    ok = set(fx.refresh_rates_from_ecb(currencies, quote))
    if ok:
        print(f"[currency] ECB: {len(ok)}/{len(currencies)} taux courants chargés.")
    ...
    fx.save_disk_cache()
    manquantes = sorted(set(currencies) - ok)
