...
@router.post("/revisions/replan")
def replan_weekly_revisions(session: SessionDep, date: Optional[dt.date] = None) -> dict:
    """Recalcule les révisions de la semaine en cours après un changement d'horaire."""
    from app.services.agenda.weekly_revisions import replan_current_weekly_revisions

    return {"message": replan_current_weekly_revisions(session, date)}
