...
class IcalSourceRequest(BaseModel):
    url: str = Field(min_length=10, max_length=2048)
    label: str = Field(default="7shifts", max_length=80)
...
def sync_ical_url(body: IcalSyncRequest, session: SessionDep):
    ...
    try:
        counts = import_ics_from_url(session, body.url)
...
@router.get("/ical-source")
def read_ical_source():
    """Retourne l'état de la source de travail sans exposer son URL secrète."""
    from app.services.agenda.ical_source import status

    return status()


@router.put("/ical-source")
def write_ical_source(body: IcalSourceRequest):
    """Enregistre localement une source iCal. L'URL n'est jamais relue par l'UI."""
    from app.services.agenda.ical_source import configure

    try:
        return configure(body.url, body.label)
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc


@router.delete("/ical-source")
def delete_ical_source():
    from app.services.agenda.ical_source import remove

    return remove()


@router.post("/ical-source/sync", response_model=ImportIcalResponse)
def sync_configured_ical_source(session: SessionDep):
    """Lance immédiatement la synchro de travail et met à jour son statut."""
    from app.services.agenda.ical_source import sync
