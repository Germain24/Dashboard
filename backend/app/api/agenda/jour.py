...
from app.services.agenda import (
    ...
    dedupe_sport_events,
    free_slots,
    get_full_calendar_with_carryover,
...
def today(session: SessionDep):
    ...
    raw_events = get_full_calendar_with_carryover(session, from_dt, to_dt)
    from app.services.agenda.conflicts import is_meal_event

    raw_events = [event for event in raw_events if not is_meal_event(event)]
...
def slots_endpoint(
    ...
    items = get_full_calendar_with_carryover(session, from_dt, to_dt)
    from app.services.agenda.conflicts import is_meal_event

    items = [event for event in items if not is_meal_event(event)]
