...
@router.post("/store-pricing/refresh-if-stale", status_code=status.HTTP_202_ACCEPTED)
def refresh_store_pricing_if_stale() -> dict:
    """Déclenche la vérification Super C sans bloquer l'ouverture de Santé."""
    from app.services.sante.fenetre_service import refresh_prices_best_effort

    return {"started": refresh_prices_best_effort()}
