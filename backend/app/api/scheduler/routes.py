...
def force_run(job_id: str):
    ...
    publish("jobs.changed", data={"job_id": job_id, "action": "triggered"}, invalidate=[["jobs"]])
    return {"status": "triggered", "job_id": job_id}

@router.post("/{job_id}/pause")
def pause_job(job_id: str):
    get_scheduler().pause_job(job_id)
    publish("jobs.changed", data={"job_id": job_id, "action": "paused"}, invalidate=[["jobs"]])
    return {"status": "paused"}

@router.post("/{job_id}/resume")
def resume_job(job_id: str):
    get_scheduler().resume_job(job_id)
    publish("jobs.changed", data={"job_id": job_id, "action": "resumed"}, invalidate=[["jobs"]])
