...
def _write_test_csv(tmp_path):
    ...
    # Toutes les colonnes de propriétés à écrire (union)
    all_props = set()
    for props in _MINI_FOODS.values():
        all_props.update(props.keys())
    props_ordered = sorted(all_props)

    lines = [";".join(["Nutriments", *aliments])]
    for prop in props_ordered:
        row = [prop] + [str(_MINI_FOODS[a].get(prop, 0)) for a in aliments]
...
def client(tmp_path, monkeypatch):
    """Client FastAPI avec une DB SQLite isolée + un aliments.csv temporaire.

    Depuis CONV 3.1, le catalogue d'aliments est lu directement depuis le CSV
    (settings.imports_dir / "aliments.csv"). On monkeypatch `imports_dir` pour
    pointer vers un dossier temporaire propre à ce test.
    """
    engine = create_engine(f"sqlite:///{tmp_path}/test.db", connect_args={"check_same_thread": False})
    SQLModel.metadata.create_all(engine)

    # Écrit le CSV de test
    _write_test_csv(tmp_path)

    # Monkeypatch settings.imports_dir pour pointer sur tmp_path/imports
    from app.core.config import settings
    monkeypatch.setattr(type(settings), "imports_dir", property(lambda self: tmp_path / "imports"))
