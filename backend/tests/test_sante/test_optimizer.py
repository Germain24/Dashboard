...
def test_minqty_does_not_force_inclusion():
    ...
    by_name = {it["Aliment"]: it for it in plan}
    if "Brocoli" in by_name:
        # MinQty est désormais seulement un plancher : le plafond journalier a
        # volontairement été beaucoup augmenté pour permettre la consolidation.
        assert by_name["Brocoli"]["Quantite_g"] >= 400.0 - 1.0
...
def test_window_quantity_limits_scale_with_number_of_days():
    df = _mini_catalog().loc[["Riz blanc"]].copy()
    df.loc["Riz blanc", "MaxQty"] = 300.0
    targets = _t(_base_targets())
    targets.update({"Calories": 3000.0, "Protéines": 55.0, "Lipides": 5.0,
                    "Glucides": 650.0, "Poids_Corps": 100.0})
    plan, _ = optimize_nutrition(
        df, targets, budget_max_daily=100.0, quantity_days=3,
    )
    assert plan is not None
    assert plan[0]["Quantite_g"] > 300.0
    assert plan[0]["Quantite_g"] <= 900.0 + 1e-6


def test_four_day_window_multiplies_daily_limit_by_four():
    df = _mini_catalog().loc[["Riz blanc"]].copy()
    df.loc["Riz blanc", "MaxQty"] = 300.0
    targets = _t(_base_targets())
    targets.update({"Calories": 3800.0, "Protéines": 70.0, "Lipides": 5.0,
                    "Glucides": 850.0, "Poids_Corps": 100.0})
    plan, _ = optimize_nutrition(
        df, targets, budget_max_daily=100.0, quantity_days=4,
    )
    assert plan is not None
    assert plan[0]["Quantite_g"] > 900.0
    assert plan[0]["Quantite_g"] <= 1200.0 + 1e-6


def test_juice_cap_is_shared_across_all_brands():
    df = _mini_catalog()
    juice = df.loc["Riz blanc"].copy()
    juice["VitC"] = 100.0
    juice["Prix"] = 0.01
    df.loc["Oasis Jus de mangue"] = juice
    df.loc["Selection Nectar de poire"] = juice
    targets = _t(_base_targets())
    targets["VitC"] = 500.0
    plan, _ = optimize_nutrition(df, targets, budget_max_daily=30.0)
    assert plan is not None
    juice_g = sum(
        item["Quantite_g"] for item in plan
        if "Jus" in item["Aliment"] or "Nectar" in item["Aliment"]
    )
    assert juice_g <= 250.0 + 1e-4


def test_creami_and_freezing_flags_do_not_limit_nutrition_optimizer():
    df = _mini_catalog()
    df["CreamiOk"] = [0.0, 1.0, 0.0, 1.0]
    df["Congelable"] = [1.0, 0.0, 1.0, 0.0]
    plan_a, _ = optimize_nutrition(df, _t(_base_targets()), budget_max_daily=18.0)
    df["CreamiOk"] = 1.0 - df["CreamiOk"]
    df["Congelable"] = 1.0 - df["Congelable"]
    plan_b, _ = optimize_nutrition(df, _t(_base_targets()), budget_max_daily=18.0)
    assert [p["Aliment"] for p in plan_a] == [p["Aliment"] for p in plan_b]
    assert [p["Quantite_g"] for p in plan_a] == pytest.approx(
        [p["Quantite_g"] for p in plan_b]
    )
...
def test_diversity_malus_reduces_marginal_filler_foods():
    ...
    # Avec les plafonds journaliers volontairement beaucoup plus permissifs,
    # SLSQP peut conserver des traces numériques de plusieurs équivalents. Le
    # malus ne doit toutefois jamais en ajouter davantage.
...
def test_tomates_sechees_de_marques_differentes_partagent_un_plafond():
    from app.services.sante.optimizer import GROUP_MAX_GRAMS_PER_DAY, _food_group

    assert _food_group("Pastene Tomates séchées au soleil") == "tomates_sechees"
    assert _food_group("Cedar Tomates séchées dans l'huile") == "tomates_sechees"
    assert GROUP_MAX_GRAMS_PER_DAY["tomates_sechees"] == 30.0
