...
def test_run_job_none_result_is_silent_but_still_logged(patched_db_engine):
    """Un job qui renvoie None n'a rien d'actionnable : aucune Notification, mais
    le JobRun est bien créé (traçabilité via /jobs). C'est ce qui débruite le
    centre de notifications (agenda_reminders, 96 passages/jour à vide)."""
    run_job("demo_silent", lambda s: None)
    with Session(patched_db_engine) as s:
        runs = s.exec(select(JobRun)).all()
        assert len(runs) == 1
        assert runs[0].status == "success"
        assert runs[0].log == "OK"
