...
def test_purge_removes_very_old_unread_notifications():
    """Les non lues sont gardées bien plus longtemps que les lues, mais pas
    éternellement : au-delà de la fenêtre longue, elles sont purgées (sinon la
    table ne cesse jamais de croître -- audit §3.E, 1 419 non lues jamais vues)."""
    s = _session()
    now = dt.datetime(2026, 6, 9, 12, 0, 0)

    s.add(Notification(titre="non lu recent", lu=False, created_at=now - dt.timedelta(days=40)))
    s.add(Notification(titre="non lu tres vieux", lu=False, created_at=now - dt.timedelta(days=200)))
    s.commit()

    res = purge_old(s, now=now, jobrun_days=30, notif_days=30, unread_days=90)
    assert res["notifications"] == 1

    titres = {n.titre for n in s.exec(select(Notification)).all()}
    assert titres == {"non lu recent"}
