...
def test_module_access_preferences_persist_for_other_devices(store):
    access = {"favorites": ["agenda", "budget"], "recents": ["travail", "agenda"]}
    store.update({"module_access": access})

    assert store.load()["module_access"] == access
