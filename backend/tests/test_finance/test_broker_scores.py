...
from app.services.finance.buffett.broker_availability import (
    update_broker_file_scores,
    update_broker_file_scores_isolated,
)
...
def test_isolated_writer_updates_workbook(tmp_path, monkeypatch):
    from app.services.finance.buffett import broker_availability

    workbook = tmp_path / "ToutBroker.xlsx"
    _make_file(workbook)
    monkeypatch.setattr(broker_availability.Config, "DATA_DIR", str(tmp_path))

    count = update_broker_file_scores_isolated(
        [_row("AAPL", chance_moat=91.0, achat=True)],
        path=str(workbook),
    )

    assert count == 1
    updated = pd.read_excel(workbook).set_index("Ticker Yahoo Finance")
    assert updated.loc["AAPL", "Chance MOAT"] == 91.0
