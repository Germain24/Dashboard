...
def test_stock_cross_listing_still_merged_by_exact_normalized_name():
    # Compatibilité avec les anciens catalogues dépourvus d'ISIN.
    returns, df = _setup([
...
def test_stock_cross_listing_uses_isin_even_when_names_differ():
    returns, df = _setup([
        {
            "Ticker": "OR.PA", "Nom": "L'Oréal S.A.", "Volume": 100,
            "Secteur": "Consumer Defensive", "ISIN": "FR0000120321",
            "Primary Market": True,
        },
        {
            "Ticker": "LOR.DE", "Nom": "LOREAL (local German label)", "Volume": 1000,
            "Secteur": "Consumer Defensive", "ISIN": "FR0000120321",
            "Primary Market": False,
        },
    ])
    out = deduplicate_tickers(returns, df)
    assert list(out.columns) == ["OR.PA"]


def test_many_distinct_stocks_are_not_fuzzy_compared_pairwise():
    # Régression performance : l'ancien code effectuait N² comparaisons floues
    # et N rescans complets du DataFrame avant même de démarrer l'optimiseur.
    n = 5_000
    tickers = [f"T{i}" for i in range(n)]
    returns = pd.DataFrame(columns=tickers)
    df = pd.DataFrame({
        _TCOL: tickers,
        "Nom": [f"Company {i} Unique" for i in range(n)],
        "Volume": [1.0] * n,
        "Secteur": ["Industrials"] * n,
...
def test_cross_listing_dedup_preserves_a_representative_for_each_broker(monkeypatch):
    """Une cotation PEA ne doit pas disparaître au profit du jumeau plus liquide."""
    from app.services.finance.buffett.config import Config
    from app.services.finance.buffett.dedup import deduplicate_tickers

    monkeypatch.setattr(Config, "BUDGET_BROKERS", {"Trading212": 1000.0, "BoursDirect2": 1000.0})
    returns = pd.DataFrame({
        "WORLD.DE": [0.01, -0.02, 0.03],
        "WORLD.PA": [0.01, -0.02, 0.03],
    })
    frame = pd.DataFrame({
        _TCOL: ["WORLD.DE", "WORLD.PA"],
        "Nom": ["Example World UCITS ETF", "Example World UCITS ETF"],
        "Secteur": ["ETF", "ETF"],
        "ISIN": ["FR0012345678", "FR0012345678"],
        "Volume": [1_000_000, 100_000],
        "Primary Market": [True, False],
        "Trading212": [True, False],
        "BoursDirect2": [False, True],
    })

    result = deduplicate_tickers(returns, frame, _TCOL)

    assert set(result.columns) == {"WORLD.DE", "WORLD.PA"}


def test_cross_listing_dedup_uses_one_ticker_when_it_covers_both_brokers(monkeypatch):
    from app.services.finance.buffett.config import Config
    from app.services.finance.buffett.dedup import deduplicate_tickers
    ...
    assert list(result.columns) == ["WORLD.DE"]


def test_correlated_context_promotes_removed_etf_after_representative_exclusion(monkeypatch):
    """Le jumeau liquide est initialement gardé, puis son remplaçant revient
    sans recalculer les corrélations quand sa composition est rejetée."""
    from app.services.finance.buffett.config import Config
    from app.services.finance.buffett.dedup import (
        apply_correlated_etf_dedup,
        prepare_correlated_etf_dedup,
    )

    monkeypatch.setattr(Config, "BUDGET_BROKERS", {"A": 1000.0})
    a, b = _perfect_twins()
    rets = pd.DataFrame({"FALLBACK": a, "PRIMARY": b, "STOCK": a})
    df = pd.DataFrame({
        _TCOL: ["FALLBACK", "PRIMARY", "STOCK"],
        "Volume": [10, 1000, 1],
        "Secteur": ["ETF", "ETF", "Technology"],
        "A": [True, True, True],
    })

    context = prepare_correlated_etf_dedup(rets, df, _TCOL, threshold=0.95)
    initial, removed = apply_correlated_etf_dedup(rets, context)
    promoted, second_removed = apply_correlated_etf_dedup(
        rets, context, excluded_tickers={"PRIMARY"}
    )

    assert list(initial.columns) == ["PRIMARY", "STOCK"]
    assert [item[0] for item in removed] == ["FALLBACK"]
    assert list(promoted.columns) == ["FALLBACK", "STOCK"]
    assert second_removed == []


def test_correlated_context_never_promotes_incompatible_broker_replacement(monkeypatch):
    from app.services.finance.buffett.config import Config
    from app.services.finance.buffett.dedup import (
        apply_correlated_etf_dedup,
        prepare_correlated_etf_dedup,
    ...
    a, b = _perfect_twins()
    rets = pd.DataFrame({"ONLY_A": a, "ONLY_B": b})
    df = pd.DataFrame({
        _TCOL: ["ONLY_A", "ONLY_B"],
        "Volume": [10, 1000],
        "Secteur": ["ETF", "ETF"],
    ...
    assert set(initial.columns) == {"ONLY_A", "ONLY_B"}
    assert removed == []
...
def test_strict_eur_conversion_never_silently_uses_native_returns(monkeypatch):
    ...
    monkeypatch.setattr(yf_module, "yf_session", lambda: None)
    monkeypatch.setattr(fx, "get_historical_rates", lambda *_args, **_kwargs: pd.Series())
...
def test_historical_fx_uses_inverse_yahoo_pair(monkeypatch):
    from app.services.finance import yf_session as yf_module
    from app.services.finance.buffett import dedup

    index = pd.date_range("2026-01-01", periods=2, freq="D")
    returns = pd.DataFrame({"CHF_ASSET": [0.0, 0.10]}, index=index)
    calls = []

    def fake_download(**kwargs):
        calls.append(kwargs["tickers"])
        if kwargs["tickers"] == "EURCHF=X":
            return pd.DataFrame({"Close": [1.0, 2.0]}, index=index)
        return pd.DataFrame()

    monkeypatch.setattr(dedup, "_ticker_currency", lambda _ticker: "CHF")
    monkeypatch.setattr(yf_module, "download_with_timeout", fake_download)
    monkeypatch.setattr(yf_module, "yf_session", lambda: None)

    converted = dedup.returns_in_base_currency(returns, "EUR", strict=True)

    assert calls == ["CHFEUR=X", "EURCHF=X"]
    assert converted.loc[index[1], "CHF_ASSET"] == pytest.approx(-0.45)


def test_historical_fx_falls_back_to_ecb(monkeypatch):
    from app.services.finance import fx
    from app.services.finance import yf_session as yf_module
    from app.services.finance.buffett import dedup
    ...
    returns = pd.DataFrame({"CHF_ASSET": [0.0, 0.10]}, index=index)
    monkeypatch.setattr(dedup, "_ticker_currency", lambda _ticker: "CHF")
    monkeypatch.setattr(yf_module, "download_with_timeout", lambda **_kwargs: pd.DataFrame())
    monkeypatch.setattr(yf_module, "yf_session", lambda: None)
    monkeypatch.setattr(
        fx,
        "get_historical_rates",
        lambda *_args, **_kwargs: pd.Series([1.0, 1.1], index=index),
    )

    converted = dedup.returns_in_base_currency(returns, "EUR", strict=True)

    assert converted.loc[index[1], "CHF_ASSET"] == pytest.approx(0.21)
