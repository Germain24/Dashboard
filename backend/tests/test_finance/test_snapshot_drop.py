...
def test_take_snapshot_now_keeps_invested_capital_after_partial_refresh(session, monkeypatch):
    """Un rafraîchissement incomplet ne doit pas faire chuter le capital affiché."""
    import app.services.finance.portfolio as portfolio_mod

    session.add(SnapshotPortefeuille(date=dt.date(2026, 1, 1), valeur=2_000.0, investit=1_500.0))
    session.commit()
    monkeypatch.setattr(
        portfolio_mod,
        "get_positions",
