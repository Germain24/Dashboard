...
def test_le_benchmark_non_eligible_ne_recoit_aucun_acces_broker(monkeypatch):
    """Une série téléchargée comme référence ne devient jamais achetable."""
    ...
    df = pd.DataFrame([
        {"Ticker Yahoo Finance": "A", "Nom": "a", "Achat": True, "IBKR": True},
        {"Ticker Yahoo Finance": "B", "Nom": "b", "Achat": True, "IBKR": True},
    ])
    access, _brokers = prepare_optimization(["A", "B", "CW8.PA"], df)
    assert access == [[True], [True], [False]]
