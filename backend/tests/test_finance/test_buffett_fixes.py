...
# ── ETF connu dans ToutBroker ou prouvé par nom + fondamentaux vides ─────────

def test_check_is_etf_uses_broker_secteur1(monkeypatch):
    """Le quoteType seul ne suffit pas, mais un vrai profil de fonds est détecté."""
    ...
    # Le faux quoteType ETF de BA.TO ne suffit pas à transformer Boeing en fonds.
    assert _check_is_etf(
        "BA.TO",
        {"info": {"quoteType": "ETF", "longName": "The Boeing Company"}},
    ) is False
    # En revanche, nom de fonds + absence de fondamentaux constitue une preuve.
    assert _check_is_etf(
        "FUND",
        {"info": {"quoteType": "ETF", "longName": "iShares World ETF"}},
    ) is True
...
def test_analyze_financials_etf_only_if_in_broker_set():
    ...
    assert score != 200.0
    # présent dans le set -> ETF ; le MOAT est N/A (0 interne, NULL en base)
...
def test_buffett_result_out_exposes_v2_score_components():
    from app.api.schemas_finance import BuffettResultOut
    from app.models.finance import BuffettRunResult
    ...
    row = BuffettRunResult(
        ...
        secteurs_extra={
            "scores": {
                ...
                "buffett_rules_score": 91.0,
                "buffett_quality_score": 88.5,
                "confidence_pct": 78.0,
                "durability_score": 84.0,
                "financial_moat_proxy_score": 86.0,
            }
        },
    )
    ...
    assert dumped["score"] == 78.2
    assert dumped["buffett_quality_score"] == 88.5
    assert dumped["score_confidence_pct"] == 78.0
    assert dumped["buffett_rules_score"] == 91.0
    assert dumped["score_model_version"] == 2
    assert dumped["score_business_model"] == "standard"
...
def test_delisted_ticker_with_stale_local_data_is_deleted(monkeypatch):
    ...
    assert ok is True  # traité définitivement, donc pas de retry en fin de passe
