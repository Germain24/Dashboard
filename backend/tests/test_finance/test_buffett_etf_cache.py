...
def test_analyze_one_etf_short_circuits_without_expensive_fetch(monkeypatch, tmp_path):
    ...
    fichier local, doit renvoyer un ETF sans jamais appeler Yahoo."""
    ...
    monkeypatch.setattr(runner.Config, "DATA_DIR", str(tmp_path))
    ...
    assert results["CW8.PA"][0] == 0.0
    assert results["CW8.PA"][1]["Secteur"] == "ETF"
    assert results["CW8.PA"][1]["Nom"] == "CW8.PA"


def test_analyze_one_etf_cache_froid_ne_fait_aucun_fetch(monkeypatch, tmp_path):
    """Le prix et le volume seront pris au download groupé de corrélation."""
    from app.services.finance.buffett import runner
    ...
    assert ok is True
    assert results["CW8.PA"][0] == 0.0
    assert "Volume" not in results["CW8.PA"][1]


def test_analyze_one_promotes_recent_action_metrics_without_network(monkeypatch, tmp_path):
    from app.services.finance.buffett import runner

    class _RecentlyReclassified(_FakeCacheMiss):
        def get_recent_metrics(self, ticker):
            return {
                "Nom": "ETF corrigé", "Prix": 42.0,
                "Volume": 250_000.0, "VolumeDevise": "EUR",
            }

    cache = _RecentlyReclassified()
    monkeypatch.setattr(runner, "_check_is_etf", lambda t, data=None: True)
    monkeypatch.setattr(runner.Config, "FOLDER_PATH", str(tmp_path))
    monkeypatch.setattr(runner.Config, "DATA_DIR", str(tmp_path))
    monkeypatch.setattr(
        runner, "_fetch_with_retry", lambda *args, **kwargs: (_ for _ in ()).throw(
            AssertionError("Yahoo ne doit pas être appelé pour une promotion récente")
        )
    )

    results = {}
    assert runner._analyze_one(
        "NEWETF.L", results, cache, object(), set(), threading.Lock()
    )
    assert results["NEWETF.L"] == (
        0.0,
        {
            "Nom": "ETF corrigé",
            "Prix": 42.0,
...
def test_secondary_quote_uses_primary_fundamentals_but_keeps_execution_ticker(
    monkeypatch,
    tmp_path,
):
    from app.services.finance.buffett import runner
    from app.services.finance.buffett.fundamentals_resolver import FundamentalsLink

    monkeypatch.setattr(runner, "_check_is_etf", lambda t, data=None: False)
    monkeypatch.setattr(runner, "load_local_data", lambda *args, **kwargs: None)
    monkeypatch.setattr(runner.Config, "FOLDER_PATH", str(tmp_path))
    monkeypatch.setattr(runner.Config, "DATA_DIR", str(tmp_path))
    ...
    def _fake_fetch(ticker, rate_limiter, stop_flag=None):
        calls.append(ticker)
        if ticker == "EF3.DE":
            return {
                "income": pd.DataFrame(),
        ...
        return {
            ...
            "balance": financials,
            "cashflow": financials,
            "info": {"quoteType": "EQUITY", "longName": "Eiffage"},
        }
    ...
    monkeypatch.setattr(
        ...
        "analyze_financials",
        lambda ticker, data: (
            scored.append(ticker) or 82.0,
            {"Nom": "Eiffage", "Achat": True},
        ),
    )
    monkeypatch.setattr(
        runner,
        "save_local_data",
        lambda ticker, data, identity_key=None: (
            saved.append((ticker, identity_key)) or True
        ),
    )
    ...
    ok = runner._analyze_one(
        ...
        set(),
        threading.Lock(),
        fundamentals_link=FundamentalsLink(
            quote_symbol="EF3.DE",
    ...
    assert ok is True
    assert calls == ["EF3.DE", "FGR.PA"]
    assert scored == ["EF3.DE"]
    assert saved == [("FGR.PA", "FR0000130452")]
    assert results["EF3.DE"][0] == 82.0
    assert results["EF3.DE"][1]["FundamentalsSymbol"] == "FGR.PA"
    assert results["EF3.DE"][1]["QuoteSymbol"] == "EF3.DE"
...
def test_save_local_data_persists_when_only_info_nonempty(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(data_fetch.Config, "DATA_DIR", str(tmp_path))
...
def test_excel_mirror_roundtrip_and_atomic_cleanup(monkeypatch, tmp_path):
    """Le miroir contient l'historique complet et ne laisse aucun temporaire."""
    from app.services.finance.buffett import data_fetch

    monkeypatch.setattr(data_fetch.Config, "FOLDER_PATH", str(tmp_path))
    dates = pd.to_datetime(["2023-12-31", "2024-12-31", "2025-12-31"])
    data = {
        "income": pd.DataFrame({"Net Income": [10.0, 12.0, 15.0]}, index=dates),
        "balance": pd.DataFrame({"Total Assets": [100.0, 110.0, 125.0]}, index=dates),
        "cashflow": pd.DataFrame({"Free Cash Flow": [8.0, 11.0, 14.0]}, index=dates),
        "info": {"longName": "Example", "companyOfficers": [{"name": "A"}]},
    }

    assert data_fetch.save_excel_mirror("TEST.PA", data) is True
    target = tmp_path / "TEST.PA.xlsx"
    assert target.exists()
    loaded = pd.read_excel(target, sheet_name=None, index_col=0)
    assert list(pd.to_datetime(loaded["income"].index).year) == [2023, 2024, 2025]
    assert loaded["income"]["Net Income"].tolist() == [10, 12, 15]
    assert not list(tmp_path.glob(".TEST.PA-*.xlsx"))
