...
def test_execution_quote_is_stored_on_single_primary_result(mem_session):
    mem_session.add(
        BuffettRunResult(run_id=2, ticker="FGR.PA", chance_moat=82.0)
    )
    mem_session.add(
        BuffettRunResult(run_id=2, ticker="EF3.DE", chance_moat=82.0)
    )
    mem_session.commit()

    update_allocations(mem_session, 2, [{
        "Ticker": "EF3.DE",
    ...
    primary = mem_session.exec(
        select(BuffettRunResult)
        .where(BuffettRunResult.run_id == 2)
        .where(BuffettRunResult.ticker == "FGR.PA")
    ).one()
    secondary = mem_session.exec(
        select(BuffettRunResult)
        .where(BuffettRunResult.run_id == 2)
        .where(BuffettRunResult.ticker == "EF3.DE")
    ).one()
    assert primary.allocation_pct == 10.0
    assert primary.secteurs_extra["allocations"][0]["ticker"] == "EF3.DE"
    assert secondary.allocation_pct is None
