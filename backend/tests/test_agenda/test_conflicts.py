...
def test_meals_do_not_create_calendar_conflicts():
    existing = [
        {"id": 1, "titre": "Déjeuner", "debut": _dt(10), "fin": _dt(11)},
        {"id": 2, "titre": "Réunion", "debut": _dt(10), "fin": _dt(11)},
    ]
    conflicts = find_conflicts(_dt(10), _dt(10, 30), existing)
    assert [event["titre"] for event in conflicts] == ["Réunion"]
