...
def test_meals_are_omitted_from_calendar_event_list(client):
    for title in ("Déjeuner", "Dîner"):
        client.post("/agenda/events", json={
            "titre": title,
            "debut": "2026-09-10T12:00:00",
            "fin": "2026-09-10T12:30:00",
            "source": "planner",
        })
    client.post("/agenda/events", json={
        "titre": "Cours",
        "debut": "2026-09-10T13:00:00",
        "fin": "2026-09-10T14:00:00",
    })

    response = client.get(
        "/agenda/events?from=2026-09-10T00:00:00&to=2026-09-11T00:00:00&include_training=false"
    )
    assert response.status_code == 200
    assert [event["titre"] for event in response.json()] == ["Cours"]


def test_overnight_sleep_is_returned_for_wake_day_and_blocks_free_slots(client):
    today = dt.date.today()
    yesterday = today - dt.timedelta(days=1)
    created = client.post("/agenda/events", json={
        "titre": "Sommeil",
        "debut": f"{yesterday.isoformat()}T23:00:00",
        "fin": f"{today.isoformat()}T08:00:00",
        "categorie": "autre",
        "source": "planner",
    })
    ...
    events = client.get(
        f"/agenda/events?from={today.isoformat()}T00:00:00&to={today.isoformat()}T23:59:59&include_training=false"
    ).json()
    assert any(event["titre"] == "Sommeil" for event in events)

    day = client.get("/agenda/today").json()
    assert any(event["titre"] == "Sommeil" for event in day["evenements"])
    assert day["slots_libres"][0]["debut"].endswith("08:00:00")
