...
import { Clock, Users, Carrot, Plus, X, Trash2, Star } from 'lucide-react'
import {
  fetchRecipes,
  fetchRecipe,
  createRecipe,
          ...
          {filtered.map((r) => {
            ...
            return (
              ...
              >
                <button
                  type="button"
                  onClick={(e) => void handleToggleFav(e, r.id)}
...
function RecipeDetailModal({ id, onClose }: { id: number; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    fetchRecipeNote(id).then((n) => { if (!cancelled) setNote(n) }).catch(() => {})
    return () => { cancelled = true }
  }, [id])
