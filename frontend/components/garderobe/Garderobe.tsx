...
import { type SlotInfo, type SuggestResponse, type Vetement } from "@/lib/garderobe";
import {
...
const TAB_PANEL_ID = "garderobe-tab-panel";
...
export function Garderobe() {
  const [tab, setTab] = useTabParam<Tab>("inventaire");
  ...
  const error =
    actionError ?? (firstError ? (firstError.error?.message ?? "Erreur de chargement") : null);
  ...
  const onValider = () => {
    ...
    validerMutation.mutate(
      { tenue: payload, use_body: useBody },
      {
        onSuccess: () => {
          onReset();
  ...
  return (
    <div className="space-y-0">
      ...
      <ModuleTabPanel panelId={TAB_PANEL_ID} activeTab={tab}>
        {loading ? (
          ...
          <>
            {weather && (
              <div className="px-6 pt-6">
                <WeatherBanner
                  weather={weather}
                  meanTemp={suggestion?.mean_temp ?? weather.mean_window_temp}
                />
              </div>
            )}
            ...
            <div key={tab} className="p-6 animate-fade-in-up">
                      ...
                      (sid) =>
                        slotsMap[sid] && (
                          <SlotCard
                            ...
                            slot={slotsMap[sid]}
                            item={tenue[sid] ?? null}
                            candidates={wardrobe.filter((v) =>
                              slotsMap[sid].categories.includes(v.categorie),
                            )}
              ...
              {tab === "inventaire" && (
                <InventaireTab wardrobe={wardrobe} onReload={() => void wardrobeQ.refetch()} />
              )}
