...
function tone(score: number): { color: string; label: string } {
  if (score >= 60) return { color: 'var(--success)', label: 'Excellent' }
  if (score >= 40) return { color: 'var(--warning)', label: 'Moyen' }
  return { color: 'var(--destructive)', label: 'À surveiller' }
}
...
export function ScoreRingModule() {
  ...
  const t = score != null ? tone(score) : { color: 'var(--muted)', label: 'Pas encore de données' }
  const comps = data?.composantes

  return (
    <Link
      href="/score"
      aria-label={`Score de forme${score != null ? ` : ${Math.round(score)} sur 100 (${t.label})` : ''}`}
    ...
    >
      <motion.div ref={ref} style={{ x, y }} className="relative h-[220px] w-[220px]">
        <svg viewBox="0 0 200 200" className="h-full w-full -rotate-90">
          ...
          <motion.circle
            cx="100" cy="100" r={R} fill="none" stroke={t.color} strokeWidth="10" strokeLinecap="round"
