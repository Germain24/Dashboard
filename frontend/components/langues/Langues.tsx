...
const TAB_PANEL_ID = "langues-tab-panel";

export function Langues() {
  const [onglet, setOnglet] = useTabParam<Onglet>("japonais");
  ...
  return (
    <div className="space-y-0">
      ...
      <ModuleTabPanel panelId={TAB_PANEL_ID} activeTab={onglet} className="p-6 animate-fade-in-up">
        {onglet === "japonais" ? <JaponaisTab /> : <ProjetsTab />}
      </ModuleTabPanel>
    </div>
  );
}
...
function JaponaisTab() {
  ...
  if (vocabQ.isLoading) {
    return (
      <div className="h-20 rounded-xl border border-[var(--border)] bg-[var(--card)] skeleton-shimmer" />
    );
  }
  if (vocabQ.isError)
    return <div className="text-[var(--destructive)]">⚠ {vocabQ.error.message}</div>;
  ...
  const submit = () => {
    ...
    createM.mutate(
      ...
      {
        onSuccess: () =>
          setForm({ terme: "", lecture: "", traduction: "", type: form.type, tags: "" }),
      },
    );
  };
      ...
      <div className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-4 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
          <label className="block">
            ...
            <input
              value={form.terme}
              onChange={(e) => setForm({ ...form, terme: e.target.value })}
              placeholder="水, 食べる…"
              className={inputCls}
            />
          </label>
        ...
        <ul className="space-y-1.5">
          {entries.map((v) => (
            ...
            >
              ...
              <span className="rounded-md bg-[var(--muted)] px-1.5 py-0.5 text-xs">
                {v.type === "kanji" ? "Kanji" : "Vocab"}
              </span>
...
function ProjetsTab() {
  ...
  if (projetsQ.isError)
    return <div className="text-[var(--destructive)]">⚠ {projetsQ.error.message}</div>;
  ...
  const submit = () => {
    ...
    createM.mutate(
      ...
      {
        onSuccess: () =>
          setForm({ titre: "", type: "voyage", echeance: "", budget_estime: "", notes: "" }),
      },
    );
  };
      ...
      <div className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-4 space-y-3">
        ...
        <label className="block">
          ...
          <input
            ...
            placeholder="Logistique, documents, itinéraire…"
            className={inputCls}
          />
        </label>
        <div className="flex justify-end">
      ...
      {projets.length === 0 ? (
        ...
        <ul className="space-y-2">
          {projets.map((p) => (
            <li
              key={p.id}
              className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-4 space-y-1 card-hover"
            >
              <div className="flex flex-wrap items-center gap-2">
                <span className="font-medium">{p.titre}</span>
                <span className="rounded-md bg-[var(--muted)] px-2 py-0.5 text-xs">
                  {PROJET_TYPES[p.type]}
                </span>
                {p.echeance && (
                ...
                <select
                  ...
                  onChange={(e) =>
                    updateM.mutate({
                      id: p.id,
                      patch: { statut: e.target.value as ProjetInternational["statut"] },
                    })
                  }
