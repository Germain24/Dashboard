...
function SuggestionButton({ pending, onClick }: { pending: boolean; onClick: () => void }) {
  return <button onClick={onClick} disabled={pending} className="rounded border border-[var(--border)] px-3 py-1.5 text-sm hover:bg-[var(--muted)] disabled:opacity-50">{pending ? "Recherche…" : `Suggérer les ${MAX_CANDIDATS} lieux les plus proches`}</button>;
}

function SuggestionError({ message }: { message?: string }) {
  if (!message) return null;
  return <span className="text-sm text-[var(--destructive)]">{message}</span>;
}
      ...
      <CollapsibleSection title="Sélection manuelle (avancé)" defaultOpen={false}>
        <div className="space-y-6">
          <div className="flex flex-wrap items-end gap-2 rounded-lg border border-[var(--border)] p-3">
            ...
            <SuggestionButton pending={suggererMut.isPending} onClick={suggerer} />
            <SuggestionError message={suggererMut.isError ? suggererMut.error?.message ?? "Erreur" : undefined} />
          </div>
