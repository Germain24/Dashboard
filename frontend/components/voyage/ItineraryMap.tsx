...
function hasCoordinates(point: { lat: number | null; lon: number | null }): point is { lat: number; lon: number } {
  return point.lat != null && point.lon != null;
}

function stepPoints(etapes: Itineraire["etapes"]): Point[] | null {
  const points: Point[] = [];
  for (const step of etapes) {
    if (!hasCoordinates(step)) return null;
    points.push({ lat: step.lat, lon: step.lon, label: step.nom, detail: `${step.date_arrivee} → ${step.date_depart}` });
  }
  return points;
}
...
export function itineraryPoints(itineraire: Itineraire): Point[] | null {
  ...
  if (!hasCoordinates(depart) || !hasCoordinates(arrivee)) return null;

  const points: Point[] = [{ lat: depart.lat, lon: depart.lon, label: `Départ (${depart.iata})` }];
  const steps = stepPoints(etapes);
