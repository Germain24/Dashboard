  ...
  return (
    <div className="space-y-6">
      <div className="flex gap-2 items-center">
        <label className="text-sm text-[var(--muted)]">Semestre</label>
        <select className="border rounded px-2 py-1 text-sm bg-[var(--card-bg)]"
          ...
          <div className="flex justify-center">
            <svg width="140" height="140" viewBox="0 0 140 140">
              ...
              <circle cx="70" cy="70" r={radius} fill="none"
                stroke={gpaVal != null ? GPA_COLOR(gpaVal) : "var(--muted)"}
                strokeWidth="12"
                strokeDasharray={`${filled} ${circ - filled}`}
                strokeLinecap="round"
                transform="rotate(-90 70 70)" />
              <text x="70" y="66" textAnchor="middle" fontSize="24" fontWeight="700"
                fill={gpaVal != null ? GPA_COLOR(gpaVal) : "var(--muted)"}>
                {gpaVal != null ? gpaVal.toFixed(2) : "—"}
              </text>
              <text x="70" y="84" textAnchor="middle" fontSize="11" fill="var(--muted)">/ 4.3</text>
            </svg>
          </div>
          ...
          <div className="space-y-1">
            {result.detail.map(cg => (
              <div key={cg.cours_id} className="flex items-center gap-2 text-sm py-1 border-b border-[var(--border)] last:border-0">
                <span className="font-mono text-xs w-20 shrink-0 text-[var(--muted)]">{cg.code}</span>
                <span className="flex-1 text-xs truncate">{cg.nom}</span>
                {cg.note_finale != null ? (
                  <>
                    <span className="text-xs text-[var(--muted)]">{cg.note_finale}/100</span>
                    <span className="font-bold text-sm" style={{ color: GPA_COLOR(cg.points_gpa ?? 0) }}>
                      {cg.lettre}
                    </span>
                    <span className="text-xs w-8 text-right text-[var(--muted)]">{cg.points_gpa?.toFixed(1)}</span>
                  </>
                ) : (
                  <span className="text-xs text-[var(--muted)] italic">non noté</span>
                )}
              </div>
            ))}
          </div>
