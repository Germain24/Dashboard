...
import { Plus, Search, X, Clapperboard, Tv, Check, Clock, Bookmark, Eye } from "lucide-react";
...
function AddModal({ mediaType, onClose }: { mediaType: MediaType; onClose: () => void }) {
  ...
  const addManual = () => {
    ...
    addMutation.mutate(
      ...
      {
        onSuccess: () => {
          toast.success(`${manualTitre} ajouté.`);
          onClose();
        },
        onError: () => toast.error("Erreur lors de l'ajout."),
      },
  ...
  return (
    <Dialog
      open
      onClose={onClose}
      className="max-w-md rounded-xl border border-[var(--border)] bg-[var(--card)] shadow-xl"
    >
      <DialogHeader onClose={onClose} className="pb-0">
        <DialogTitle className="font-semibold">
          Ajouter un {mediaType === "film" ? "film" : "une série"}
        </DialogTitle>
      </DialogHeader>
...
function ItemCard({ item }: { item: WatchItem }) {
    ...
    <div className="flex gap-3 p-3 rounded-xl border border-[var(--border)] bg-[var(--card)] hover:border-[var(--muted-foreground)] transition-colors">
      {item.poster_url ? (
        ...
        <div className="w-12 h-16 bg-[var(--muted)] rounded-lg flex items-center justify-center flex-shrink-0">
          {item.type === "film" ? (
            <Clapperboard size={18} className="text-[var(--muted-foreground)]" />
          ) : (
            <Tv size={18} className="text-[var(--muted-foreground)]" />
          )}
        </div>
      )}
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            ...
            <div className="flex items-center gap-2 mt-0.5">
              {item.annee && (
                <span className="text-xs text-[var(--muted-foreground)]">{item.annee}</span>
              )}
              {genres.length > 0 && (
              ...
              {item.type === "serie" && item.nb_saisons && (
                <span className="text-xs text-[var(--muted-foreground)]">
                  {item.nb_saisons} saison{item.nb_saisons > 1 ? "s" : ""}
                </span>
              )}
          ...
          <button
            onClick={() =>
              deleteMutation.mutate(item.id, { onError: () => toast.error("Suppression échouée.") })
            }
    ...
    <div className="max-w-xl space-y-5">
      ...
      {stats && (
        <div className="grid grid-cols-3 gap-3">
          {[
            {
              label: mediaType === "film" ? "Films vus" : "Séries vues",
              value: mediaType === "film" ? stats.films_vus : stats.series_vues,
            },
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-12 text-[var(--muted-foreground)]">
          <div className="mb-3">
            {mediaType === "film" ? (
              <Clapperboard size={32} className="mx-auto opacity-40" />
            ) : (
              <Tv size={32} className="mx-auto opacity-40" />
            )}
          </div>
          <p className="text-sm">Aucun {mediaType === "film" ? "film" : "série"} ici.</p>
          <button
            onClick={() => setShowAdd(true)}
            className="mt-2 text-sm text-[var(--ring)] hover:underline"
          >
            Ajouter le premier
          </button>
        </div>
