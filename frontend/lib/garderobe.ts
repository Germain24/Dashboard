...
export type VetementUpdate = Partial<
  Omit<
    Vetement,
    | "id"
    | "proprete_pct"
...
export const garderobeApi = {
  ...
  deleteVetement: (id: string) =>
    fetch(`${getApiBaseUrl()}/garderobe/vetements/${encodeURIComponent(id)}`, {
      method: "DELETE",
    }).then((r) => {
  ...
  syncObjectif: () => api<{ types: number }>(`/garderobe/objectif/sync`, { method: "POST" }),
  ...
  getPlanner: (start?: string) =>
    api<WeekPlan>(`/garderobe/planner${start ? `?start=${start}` : ""}`),
