...
export const entrainementApi = {
  ...
  deleteSession: (id: number) =>
    fetch(`${getApiBaseUrl()}/entrainement/sessions/${id}`, { method: "DELETE" }).then((r) => {
  ...
  deleteSet: (seance_id: number, set_id: number) =>
    fetch(`${getApiBaseUrl()}/entrainement/sessions/${seance_id}/sets/${set_id}`, {
      method: "DELETE",
    }).then((r) => {
      if (!r.ok) throw new Error(`Delete failed: ${r.status}`);
    }),
  ...
  getProgression: (exercice_id: number, days = 90) =>
    api<ProgressionResponse>(`/entrainement/progression/${exercice_id}?days=${days}`),
  ...
  getMuscleVolume: (days = 7) => api<MuscleVolume[]>(`/entrainement/volume/muscles?days=${days}`),
  ...
  deleteCardio: (id: number) =>
    fetch(`${getApiBaseUrl()}/entrainement/cardio/${id}`, { method: "DELETE" }).then((r) => {
      if (!r.ok) throw new Error(`Delete failed: ${r.status}`);
    }),

  // Intensité (consommé par Santé en interne, exposé ici pour debug UI)
  getIntensityForDate: (date: string) => api<IntensityResponse>(`/entrainement/intensity/${date}`),
  getIntensityToday: () => api<IntensityResponse>(`/entrainement/intensity/today`),
  ...
  startMesocycle: (accumulation_weeks = 4) =>
    api<Mesocycle>(`/entrainement/mesocycle/start?accumulation_weeks=${accumulation_weeks}`, {
      method: "POST",
    }),
  stopMesocycle: () => api<Mesocycle>(`/entrainement/mesocycle/stop`, { method: "POST" }),

  // Calories par date (consommé par CONV nutrition future)
  getCaloriesForDate: (date: string) => api<CaloriesDayResponse>(`/entrainement/calories/${date}`),
};
