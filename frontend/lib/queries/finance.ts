...
export function usePerf() {
  return useQuery({
    queryKey: financeKeys.perf(),
    queryFn: () => financeApi.perf(),
    staleTime: 2 * 60 * 1000,
  });
}
...
export function useRisk() {
  return useQuery({
    queryKey: financeKeys.risk(),
    queryFn: () => financeApi.risk(),
    staleTime: 5 * 60 * 1000,
  });
}
...
export function useHistory(days = 365) {
  return useQuery({
    queryKey: financeKeys.history(days),
    queryFn: () => financeApi.history(days),
...
/** Délai jusqu'au prochain jour local, avec une petite marge après minuit. */
...
export function useSetObjectifPatrimoine() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (goal: PatrimoineGoalPatch) => financeApi.setObjectifPatrimoine(goal),
    onSuccess: () => {
