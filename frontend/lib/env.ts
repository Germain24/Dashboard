...
/** URL d'une route backend exposée par le proxy de même origine. */
export function proxyApiUrl(path: string): string {
  return `/proxy${path.startsWith("/") ? path : `/${path}`}`;
}

/**
 * Base URL du backend pour les liens et appels directs aux routes historiques.
 * Le navigateur passe par le proxy Next, ce qui permet l'accès depuis un autre
 * appareil sans faire pointer `localhost` vers le téléphone lui-même.
 */
export function getApiBaseUrl(): string {
  if (typeof window !== "undefined") return "/proxy";
  return (process.env.BACKEND_URL ?? env.NEXT_PUBLIC_API_BASE_URL).replace(/\/$/, "");
}
