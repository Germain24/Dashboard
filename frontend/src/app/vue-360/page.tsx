...
import { ModuleHeader, ModuleTabPanel, type ModuleTab } from "@/components/layout";
...
const TAB_PANEL_ID = "vue-360-tab-panel";
    ...
    <div className="space-y-6">
      {active === "synthese" && (
        <>
          ...
          <section className="rounded-[var(--radius-lg)] border border-[var(--glass-border)] bg-[var(--card)] p-5">
            ...
            {wb ? (
              <div className="flex flex-wrap items-center gap-6">
                <div>
                  <div className="font-display text-4xl text-[var(--foreground)]">
                    {Math.round(wb.score)}
          ...
          {today && (
            <section className="rounded-[var(--radius-lg)] border border-[var(--glass-border)] bg-[var(--card)] p-5">
              <h2 className="mb-3 text-xs font-semibold text-[var(--muted-foreground)]">
                Aujourd&apos;hui en bref
              </h2>
              <div className="grid grid-cols-2 gap-3 text-sm sm:grid-cols-4">
                ...
                {today.sante?.poids != null && (
                  <Metric label="Poids" value={`${today.sante.poids} kg`} />
                )}
                {today.budget && (
                  <Metric label="Dépenses" value={`${today.budget.depenses_total} $`} />
                )}
              </div>
            </section>
          )}
        ...
        <>
          ...
          <section className="rounded-[var(--radius-lg)] border border-[var(--glass-border)] bg-[var(--card)] p-5">
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              {(
                [
                  ["Réussites", insights?.reussites],
                  ["Vigilance", insights?.vigilance],
                  ["Tendances", insights?.tendances],
                ] as const
              ).map(([title, items]) => (
                <div key={title}>
                  <p className="mb-1 text-xs font-semibold text-[var(--muted-foreground)]">
                    {title}
                  </p>
              ...
              <ul className="space-y-1.5">
                {corr.correlations.slice(0, 5).map((c) => (
                  ...
                  >
                    ...
                    <span
                      className={`shrink-0 tabular-nums text-xs font-medium ${c.r >= 0 ? "text-[var(--success-foreground)]" : "text-[var(--warning-foreground)]"}`}
                    >
                      {c.r > 0 ? "+" : ""}
                      {c.r.toFixed(2)}
                    </span>
        ...
        {jours.map((j) => (
          <li key={j.date} className="text-sm">
            ...
            <span className="ml-2 text-xs text-[var(--muted-foreground)]">
              {j.load_h} h · {j.n_events} évén. — {j.suggestion}
            </span>
          </li>
        ))}
      </ul>
          ...
          return (
            <li key={f.metric} className="flex items-center justify-between gap-3 text-sm">
              <span className="min-w-0 truncate text-[var(--foreground)]">
                {f.metric}{" "}
                <span className="text-xs text-[var(--muted-foreground)]">
                  {f.courant} → {f.prevision}
                </span>
              </span>
              <span
                className={`shrink-0 tabular-nums text-xs font-medium ${f.direction === "stable" ? "text-[var(--muted-foreground)]" : up ? "text-[var(--success-foreground)]" : "text-[var(--warning-foreground)]"}`}
              >
                {f.variation > 0 ? "+" : ""}
                {f.variation}
              </span>
      ...
      <ul className="space-y-2">
        {recs.map((r, i) => (
          <li key={i} className="flex items-center gap-3 text-sm">
            <span
              className="flex h-7 w-9 shrink-0 items-center justify-center rounded-[var(--radius)] bg-[var(--accent)] text-xs font-semibold tabular-nums text-[var(--foreground)]"
              title="impact estimé"
            >
              {r.impact}
            </span>
...
export default function Vue360Page() {
  const [active, setActive] = useTabParam<TabId>("synthese");
  return (
    <div>
      <ModuleHeader
        title="Vue 360"
      ...
      />
      <ModuleTabPanel panelId={TAB_PANEL_ID} activeTab={active} className="p-6 animate-fade-in-up">
