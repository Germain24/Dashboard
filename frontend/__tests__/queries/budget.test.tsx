...
vi.mock("@/lib/budget", () => ({
  ...
  createBudgetCategory: vi.fn().mockResolvedValue({ id: 2, nom: "Loyer", parent_id: 1 }),
  setTransactionCategory: vi.fn().mockResolvedValue({ id: 1, category_id: 2 }),
  ...
  fetchForecast: vi.fn().mockResolvedValue({
    moyenne_revenus: 3000,
    moyenne_depenses: 2000,
    solde_mensuel_moyen: 1000,
    points: [{ mois: "2026-08", solde_mensuel: 1000, cumul: 1000 }],
  }),
  fetchSubscriptionAlerts: vi.fn().mockResolvedValue({
    hausses: [
      {
        marchand: "NETFLIX.COM",
        montant_precedent: 15.99,
        montant_actuel: 18.99,
        delta: 3,
        delta_pct: 18.8,
        date: "2026-04-05",
        occurrences: 4,
  ...
  fetchFire: vi
    .fn()
    .mockResolvedValue({ taux_epargne_pct: 25, annees_restantes: 17.1, atteint: false }),
...
describe("queries/budget", () => {
  ...
  it("catégorise une transaction et crée une sous-catégorie", async () => {
    const assignment = renderHook(() => useSetTransactionCategory(), { wrapper });
    assignment.result.current.mutate({ id: 1, category_id: 2 });
    await waitFor(() => expect(assignment.result.current.isSuccess).toBe(true));

    const create = renderHook(() => useCreateBudgetCategory(), { wrapper });
    create.result.current.mutate({ nom: "Loyer", parent_id: 1 });
    await waitFor(() => expect(create.result.current.isSuccess).toBe(true));
  });

  it("useForecast charge la prévision de trésorerie", async () => {
    const { result } = renderHook(
      () => useForecast(6, 6, { revenusDeltaPct: 0, depensesDeltaPct: 0 }),
      { wrapper },
    );
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
