...
describe("TodayPanel — séance d'entraînement planifiée (#00h00)", () => {
  ...
  it("met en avant le prochain bloc et permet de réserver un créneau libre", () => {
    const debut = new Date(Date.now() + 60 * 60 * 1000);
    ...
    mockData = {
      date: isoLocal(debut).slice(0, 10),
      evenements: [
        {
          id: 8,
          titre: "Cours de macroéconomie",
      ...
      seance_entrainement: null,
      slots_libres: [
        {
          debut: isoLocal(new Date(fin.getTime() + 30 * 60_000)),
          fin: isoLocal(new Date(fin.getTime() + 2 * 60 * 60_000)),
          duree_min: 90,
        },
      ],
      taches_urgentes: [],
    ...
    expect(screen.getByRole("link", { name: /Planifier un bloc/i })).toHaveAttribute(
      "href",
      "/agenda",
    );
  });
