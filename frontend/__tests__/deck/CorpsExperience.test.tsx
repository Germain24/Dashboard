...
beforeEach(() => {
  vi.mocked(useScore).mockReturnValue({
    data: {
      date: "2026-06-24",
      score: 78,
      composantes: { sommeil: 80, sport: 70, nutrition: 84 },
      details: { sommeil_h: 7.5, sessions_7j: 4, kcal_consommees: 1840, kcal_cible: 2100 },
    },
    isLoading: false,
    isError: false,
  } as ReturnType<typeof useScore>);
  vi.mocked(useWaterToday).mockReturnValue({
    data: { date: "2026-06-24", eau_ml: 1500, cible_ml: 2500, pct: 60 },
    isLoading: false,
    isError: false,
  } as ReturnType<typeof useWaterToday>);
  vi.mocked(useEntrainementToday).mockReturnValue({
    data: {
      date: "2026-06-24",
      weekday: 2,
      jour_label: "Push",
      programme_jour_id: 1,
      slots: [{ label: "Développé couché" }],
      seance_en_cours: null,
      kcal_estimees: 0,
      poids_corps_kg: 70,
      mesocycle: null,
    },
  ...
  vi.mocked(useSkincareToday).mockReturnValue({
    data: { date: "2026-06-24", AM: [], PM: [{}], due: [{}] },
    isLoading: false,
    isError: false,
  ...
  it("compose le titre Corps et les 5 modules", () => {
    render(
      <MotionConfig reducedMotion="always">
        <CorpsExperience index={2} accent="#536252" />
      </MotionConfig>,
    );
