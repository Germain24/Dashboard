...
// de la semaine, non reflétés sur le prix courant superc.ca). store_pricing.py
// compare ce cache au prix superc.ca courant et retient le moins cher (voir
// _best_price).
//
...
// scraping d'image / OCR). Nom retenu en FRANÇAIS (productFr en priorité,
// productEn en repli) pour matcher les mots-clés FR du catalogue (repoint
// 2026-07-23, cf. orchestration/a-faire/2026-07-23-superc-ca-repoint-design.md).
// Schéma de sortie identique aux autres scrapers.
//
  ...
  for (const pg of (pagesBody || [])) {
    for (const bl of (pg.blocks || [])) {
      for (const p of (bl.products || [])) {
        const name = (p.productFr || p.productEn || '').trim();
