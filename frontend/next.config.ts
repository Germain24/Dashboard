...
const nextConfig: NextConfig = {
  ...
  rewrites() {
    return Promise.resolve([
      {
        // Les requêtes du navigateur restent sur la même origine. Le proxy
        // couvre les routes versionnées (/api/v1/...) et les anciennes routes
        // racine (/sante, /media, etc.) utilisées par certains modules.
        source: "/proxy/:path*",
        destination: `${BACKEND_URL}/:path*`,
      },
      {
        // Compatibilité avec les clients historiques qui appellent encore
        // /api/<module>. L'API FastAPI versionnée est la source de vérité ;
        // cette réécriture évite qu'une migration frontend incomplète ne
        // transforme ces appels en 404 Next.js.
        source: "/api/v1/:path*",
