...
.PHONY: help install install-backend install-frontend dev dev-reload dev-stop dev-backend dev-frontend \
        migrate migrate-new import seed test test-backend test-frontend gen-types \
        clean fmt lint hooks wait-health up down prod oracle-up oracle-down
...
help:
	...
	@echo "  dev-reload         -> idem avec rechargement automatique Uvicorn"
	@echo "  dev-stop           -> arrête proprement tous les processus du dashboard"
...
# ---------- DEV ----------
dev:
	powershell -NoProfile -ExecutionPolicy Bypass -File scripts/dev-processes.ps1 -Action Check
	cd frontend && npx concurrently --kill-others-on-fail -n backend,frontend -c blue,magenta \
	  "cd ../backend && uv run uvicorn app.main:app --host 127.0.0.1 --port 8000" \
	  "cd ../backend && uv run python scripts/wait_for_health.py --url http://127.0.0.1:8000/api/v1/health --timeout 120 && cd ../frontend && next dev --hostname $(FRONTEND_HOST)"
...
dev-stop:
	powershell -NoProfile -ExecutionPolicy Bypass -File scripts/dev-processes.ps1 -Action Stop
