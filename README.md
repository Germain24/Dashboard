...
Documentation ciblée : [backend](backend/README.md), [frontend](frontend/README.md)
et [déploiement Oracle](deployment/oracle/README.md).
...
sert le frontend et conserve le proxy `/proxy/*` vers le backend ; les routes
`/api/*` sont réécrites vers l'API versionnée. Le backend tourne avec
...
| `make wait-health`    | Attend que `/api/v1/health` réponde (post-démarrage) |
...
`orchestration/PLAN.md` (conversations de build) et `orchestration/finis/AMELIORATIONS_200.txt`
