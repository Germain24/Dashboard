...
- **Données** : TanStack Query (cache, retries, invalidation). Le cache reste en
  mémoire : `QueryProvider` supprime volontairement la persistance localStorage
  (données privées). Le client HTTP `lib/api.ts` ajoute timeout + AbortController
  et tolère les réponses vides (204).
